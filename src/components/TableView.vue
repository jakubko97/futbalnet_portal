<template>
    <v-data-table mobile-breakpoint="0" hide-default-footer :headers="headers" :items="teams" :items-per-page="24"
        class="elevation-0">
        ...
        initTable() {
            ...
            Array.from(this.tableData, t => {
                ...
                Array.from(t.teams, (s, index) => {
                    let team = null
                    if (index == 0) {
                        team = this.teams.filter(f => f.club_name === s.name)[0]
                        team.score[0] += t.score[0]
                        team.score[1] += t.score[1]
                        team.matches += 1
                    } else if (index == 1) {
                        team = this.teams.filter(f => f.club_name === s.name)[0]
                        team.score[0] += t.score[1]
                        team.score[1] += t.score[0]
                        team.matches += 1
                    }
